...
> 当前版本：`0.2.7`。客户端界面支持 10 种语言并可跟随系统或即时切换；Windows 安装器会在全新安装和升级时显示 10 种语言选择，macOS“本地网络”权限说明也提供对应本地化。
...
| 能力 | 说明 |
| --- | --- |
| 自动发现 | 通过 mDNS、兼容 mDNS 和局域网信标持续发现 Windows/macOS 设备；启动监听就绪后立即执行首轮广播、普通扫描和好友地址优先探测，同时保留 TUN/代理路径作为回退。发现或添加一个好友不会停止后续扫描。 |
...
- 启动后不等待周期定时器，监听端口就绪即触发首轮发现；修改昵称后会立即向当前连接设备同步，不再等待空闲连接重建。
...
Version 0.1.x peers remain supported through legacy v1 with its existing 2 GiB per-file maximum; resumable v2 requires v0.2.0 or later on both peers. Automatic file receiving is optional and disabled by default. It works only for accepted friends, uses a user-selected folder, sanitizes unsafe names, numbers duplicates, and never overwrites an existing file. Version 0.2.7 provides the complete client interface in ten languages, starts LAN discovery as soon as its listener is ready, and pushes nickname changes immediately to connected peers instead of waiting for an idle reconnect. Completed image and file records offer Open, Save as, and Show in folder actions. Continuous discovery, PeerId-only friendship, notifications, version display, and verified GitHub updates remain available. Supported 0.1.x and 0.2.x versions remain compatible for discovery and friend requests.
...
La versión 0.2.7 ofrece toda la interfaz en diez idiomas, inicia la detección de la red local al abrirse y sincroniza de inmediato los cambios de nombre con los dispositivos conectados. Cada imagen o archivo completado permite Abrir, Guardar como o Mostrar en la carpeta. El descubrimiento continuo, la identidad por PeerId, las notificaciones y las actualizaciones oficiales verificadas se mantienen.
...
La version 0.2.7 propose toute l’interface en dix langues, lance immédiatement la découverte du réseau local au démarrage et synchronise sans délai les changements de nom avec les appareils connectés. Chaque image ou fichier terminé peut être ouvert, enregistré sous un autre nom ou affiché dans son dossier. La découverte continue, l’identité PeerId, les notifications et les mises à jour officielles vérifiées restent disponibles.
...
Version 0.2.7 bietet die vollständige Oberfläche in zehn Sprachen, startet die LAN-Erkennung sofort nach dem Öffnen und synchronisiert Namensänderungen direkt mit verbundenen Geräten. Abgeschlossene Bilder und Dateien lassen sich öffnen, unter einem neuen Namen speichern oder im Ordner anzeigen. Dauerhafte Erkennung, PeerId-Identität, Benachrichtigungen und geprüfte offizielle Updates bleiben erhalten.
...
A versão 0.2.7 oferece toda a interface em dez idiomas, inicia a descoberta da rede local imediatamente ao abrir e sincroniza alterações de nome com os dispositivos conectados sem demora. Imagens e arquivos concluídos podem ser abertos, salvos como outro arquivo ou mostrados na pasta. A descoberta contínua, a identidade por PeerId, as notificações e as atualizações oficiais verificadas permanecem disponíveis.
...
Версия 0.2.7 предоставляет весь интерфейс на десяти языках, запускает поиск в локальной сети сразу после открытия и без задержки синхронизирует изменения имени с подключёнными устройствами. Завершённые изображения и файлы можно открыть, сохранить под другим именем или показать в папке. Непрерывное обнаружение, идентификация по PeerId, уведомления и проверенные официальные обновления сохранены.
...
バージョン 0.2.7 ではアプリ全体を 10 言語で利用でき、起動時に LAN 検出を直ちに開始し、接続中の端末へ名前変更を即時同期します。転送済みの画像やファイルは、開く・名前を付けて保存・フォルダーで表示をチャット履歴から実行できます。継続検出、PeerId による識別、通知、検証済み公式更新も引き続き利用できます。
...
버전 0.2.7은 전체 인터페이스를 10개 언어로 제공하고 앱을 열면 LAN 검색을 즉시 시작하며 연결된 장치에 이름 변경을 바로 동기화합니다. 전송이 끝난 이미지와 파일은 채팅 기록에서 열기, 다른 이름으로 저장, 폴더에서 보기를 사용할 수 있습니다. 지속 검색, PeerId 식별, 알림, 검증된 공식 업데이트도 그대로 유지됩니다.
...
يوفّر الإصدار 0.2.7 الواجهة كاملةً بعشر لغات، ويبدأ اكتشاف الشبكة المحلية فور تشغيل التطبيق، ويزامن تغييرات الاسم مباشرةً مع الأجهزة المتصلة. يمكن فتح الصور والملفات المكتملة أو حفظها باسم أو إظهارها في المجلد مباشرةً من سجل الدردشة. يستمر الاكتشاف الدائم والتعرّف بواسطة PeerId والإشعارات والتحديثات الرسمية المتحقق منها.
