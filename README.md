...
自动接收默认关闭。开启后也只接受已经添加为好友的设备；陌生设备仍然无法发送文件。接收请求到达时，如果目录已被删除、不可写或暂时不可用，本次文件会回到手动确认流程。传输过程中不会覆盖已有文件；对于 v2 传输，遇到上述目标目录问题时会暂停并保留已确认进度；当目标目录恢复可写且可用后，会自动重试并继续传输。旧版 v1 传输不支持续传，恢复目录后需重新发送。
